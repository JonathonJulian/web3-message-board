...
function get_vm_configs() {
  # If file exists, read it, otherwise create empty config
  if [[ -f "$TFVARS_FILE" ]]; then
    VM_CONFIGS=$(jq -r '.vm_configs // {}' "$TFVARS_FILE")
  else
    echo "Error: terraform.tfvars.json not found at $TFVARS_FILE"
    echo "Creating a new file with empty vm_configs"
    mkdir -p "$CONFIG_DIR"
...
function update_vm_configs() {
  ...
  # Create a temporary file with the updated configurations
  local tmp_file="$(mktemp)"

  if [[ -f "$TFVARS_FILE" ]]; then
    # Update existing file (preserving other variables if present)
    jq --arg configs "$vm_configs" '.vm_configs = ($configs | fromjson)' "$TFVARS_FILE" > "$tmp_file"
  else
    # Create new tfvars file
    echo "{\"vm_configs\": $vm_configs}" > "$tmp_file"
  fi
...
function add_vm() {
  ...
  if echo "$vm_configs" | jq -e --arg key "$vm_key" 'has($key)' > /dev/null; then
    echo "Warning: VM '$vm_key' already exists. Updating its configuration."
  fi
...
function list_vms() {
  # Ensure the file exists before trying to list
  if [[ ! -f "$TFVARS_FILE" ]]; then
  ...
  # Check if the vm_configs is empty
  if [[ "$vm_configs" == "{}" ]]; then
    echo "No VMs configured yet."
    return
  fi
...
function remove_vm() {
  ...
  if [[ ! -f "$TFVARS_FILE" ]]; then
    echo "Error: terraform.tfvars.json not found."
    exit 1
  fi
...
function apply_config() {
  ...
  if [[ ! -f "$TFVARS_FILE" ]]; then
    echo "Error: terraform.tfvars.json not found. Add VMs before applying."
    exit 1
  fi
